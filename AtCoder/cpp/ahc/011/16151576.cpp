...
pair<string, ll> localSearch(string route, ll target_group_root)
{
  ...
  {
    score = calcSizeOf(target_group_root);
...
pair<string, ll> step(pair<ll, vl> initial_group)
{
  ...
  string ans_route = tryMoveTo(to_x, to_y);
  ...
  while((ll)ans_route.length() < t)
  {
    ll next_group_size = now_group_size;
    string before_route = ans_route;
    string init_route = moveNextOrbitalPosition(target_group_root, ans_route.back(), clockwise);
    string next_route = "";
    ...
    for (auto r : search_route)
    {
      auto [route, score] = localSearch(r, target_group_root);
      if (chmax(next_group_size, score)) next_route = route; 
    }
    ...
    {
      not_update_count++;
      if ((ll)ans_route.length() < t / 2 && not_update_count > randXor() % t)
      {
        char unit[4] = { 'L', 'U', 'R', 'D' };
        next_route = string(1 + (randXor() % n) / 2, unit[randXor() % 4]);
        not_update_count = 0;
        // clockwise = !clockwise;
      }
    }
    ...
    if ((ll)(ans_route + init_route + next_route).length() <= t)
    {
      ans_route += init_route + tryMove(next_route);
      now_group_size = next_group_size;
    }
    else tryMove(init_route, true);

    // 変わってないならバイバイ
    if (ans_route == before_route) break;
  }
  return make_pair(ans_route, calcScore());
}
...
void solve()
{
  ...
  /*
    for g in uf.groups():
    連結成分の周囲を回る
    周り1マスを local search して、点数の高いものを採用する
      点数が同じ時は、外向きの辺が多いものを採用する
    local search を dfs っぽくやるか bfs っぽくやるかは要検討

    メモ：
      作りたい形
      for (auto g : uf.groups())
      {
        auto [nearest_x, nearest_y] = nearest(g, zero_x, zero_y);
        moveTo(nearest_x, nearest_y);
        while(loop_count < t)
        {
          moveNextOrbitalPosition();
          localSearch(depth = 1)
        }
      }
  */
